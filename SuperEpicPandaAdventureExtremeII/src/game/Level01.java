...
public class Level01 extends BasicGameState {
	...
	private static float friction, acceleration, maxSpeed, gravity, jumpA;
	...
	private static void checkInGameMenu() {
		if (input.isKeyPressed(Input.KEY_ESCAPE)) {
			isMenuUp = true;
			if (oneHit3 == 0 & Game.isMusicOn) Game.marioPause.play();
			oneHit3 = -1;
		}
	}
	...
	private static void renderInfo(GameContainer gc, StateBasedGame sbg, Graphics g) {
		...
		g.drawString("v,a : " + xV + " , " + acceleration, 100, 30);
	}
	...
	private static void updateJumpingFalling(GameContainer gc, StateBasedGame sbg, int t) {
		...
		if (isJumping && !isFalling) {
			if (y <= 1) {
				...
				y -= .07;
			} else if (!won && map.getTileId((int) x, (int) (y - (pandaHeight / (tileHeight * zoomFactor)) - .07f), objectLayer) == 0
	...
	private static void updateMovement(GameContainer gc, StateBasedGame sbg, int t) {
		...
		// friction
		if (xV > 0 && !isJumping && !isFalling && isUnderfoot) xV -= friction * t;
		if (xV < 0 && !isJumping && !isFalling && isUnderfoot) xV += friction * t;
		if (!(input.isKeyDown(Input.KEY_LEFT) || input.isKeyDown(Input.KEY_RIGHT))) if (Math.abs(xV) < .00001f) xV = 0f;
		...
		if (input.isKeyDown(Input.KEY_LEFT) && !input.isKeyDown(Input.KEY_DOWN)) {
			...
			if (map.getTileId((int) (x - (xV * t) + (pandaWidth / (tileWidth * zoomFactor))), (int) (y), objectLayer) == 0
					&& map.getTileId((int) (x - (xV * t) + (pandaWidth / (tileWidth * zoomFactor))),
							(int) (y - (pandaHeight / (tileHeight * zoomFactor))), objectLayer) == 0)
				System.out.println("left is good! " + t);
			xV -= acceleration * t;
		}
		...
		if (input.isKeyDown(Input.KEY_RIGHT) && !input.isKeyDown(Input.KEY_DOWN)) {
			...
			if (map.getTileId((int) (x + (xV * t) + (pandaWidth / (tileWidth * zoomFactor))), (int) (y), objectLayer) == 0
					&& map.getTileId((int) (x + (xV * t) + (pandaWidth / (tileWidth * zoomFactor))),
							(int) (y - (pandaHeight / (tileHeight * zoomFactor))), objectLayer) == 0)
				System.out.println("Right is good! " + t);
			xV += acceleration * t;
		}
		...
		if (xV > 0) if (map.getTileId((int) (x + .02f + (pandaWidth / (tileWidth * zoomFactor))), (int) (y), objectLayer) != 0
				|| map.getTileId((int) (x + .02f + (pandaWidth / (tileWidth * zoomFactor))),
						(int) (y - (pandaHeight / (tileHeight * zoomFactor))), objectLayer) != 0) {
			xV = 0;
		}
